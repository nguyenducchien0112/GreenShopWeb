...
<head>
  ...
  <style>
    /* Ví dụ cho màn hình desktop lớn */
    @media (min-width: 1200px) {
      .col-lg-3 {
        display: block; /* Hiển thị thanh bên trên desktop lớn */
      }
      .col-md-9 {
        width: 75%; /* Điều chỉnh chiều rộng nội dung chính */
      }
    }

    /* Ví dụ cho màn hình desktop vừa */
    @media (max-width: 1199px) and (min-width: 992px) {
      .col-md-4 {
        display: block; /* Hiển thị thanh bên trên desktop vừa */
      }
      .col-md-8 {
        width: 66.666667%; /* Điều chỉnh chiều rộng nội dung chính */
      }
    }

    /* Ví dụ cho màn hình tablet trở xuống */
    @media (max-width: 991px) {
      .col-md-4 {
        display: none; /* Ẩn thanh bên trên tablet và mobile */
      }
      .col-md-8 {
    ...
    /* Hoặc có thể dùng .col-lg-3 thay vì .col-md-4 tùy thuộc vào Bootstrap */
    @media (max-width: 991px) {
      .col-lg-3 {
        display: none;
      }
    ...
    .item-grid {
      ...
      gap: 1rem; /* Optional: Add some spacing between items */
    }

    .col-xl-4, .col-lg-6 {
      /* Remove the fixed column widths as the grid will handle sizing */
      width: auto;
    }
    ...
    .card {
      ...
      height: 100%; /* Make cards take up the full height of the grid cell */
    }

    .img-wrap {
      flex-grow: 1; /* Allow image area to grow and take available vertical space */
      display: flex;
      align-items: center;
    ...
    .info-wrap {
      /* Keep the info-wrap as is or adjust styling as needed */
      padding-top: 1rem;
    }
  </style>
</head>
          ...
          <form action="${pageContext.request.contextPath}/category" method="get">
            <article class="filter-group">
              <header class="card-header my-1">
                <a data-bs-toggle="collapse" href="#collapse_category" aria-expanded="true"
              ...
              <div class="filter-content collapse show" id="collapse_category">
                <div class="card-body pt-0">
                  <c:choose>
                    <c:when test="${not empty requestScope.allCategories}">
                      <c:forEach var="categoryFilter" items="${requestScope.allCategories}" varStatus="status">
                        <c:if test="${categoryFilter.id != requestScope.category.id}">
                          <div class="form-check">
                            <input class="form-check-input" type="checkbox"
                                   value="${categoryFilter.id}"
                                   id="checkbox_category_${status.index}"
                                   name="checkedCategories"
                              ${requestScope.checkedCategories.contains(categoryFilter.id) ? 'checked' : ''} >
